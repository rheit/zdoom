	...
	else
	{
		...
		if (lump_name > lump_wad && lump_name > lump_map && lump_name != -1)
		{
			...
			if(!map->isText && stricmp(nextlumpname, "THINGS") != 0)
			{
				...
				{
					map->resource = FResourceFile::OpenResourceFile(Wads.GetLumpFullName(lump_name), Wads.ReopenLumpNum(lump_name), true);
					wadReader = map->resource->GetReader();
				}
			}
			else
...
void P_LoadThingsDoom64 (MapData * map)
{
	...
	for(int i = 0; i< numthings; i++)
	{
		...
		// Camera hack
		if (mti[i].EdNum == 0 && mth[i].thingid > 0)
			mti[i].EdNum = 9025; // Basic security camera
		if (mti[i].thingid)
			Printf("Thing type %i (%s) has tid %i and flags %x\n",
			mti[i].EdNum, mti[i].info && mti[i].info->Type
				?mti[i].info->Type->TypeName.GetChars()
